#ifndef EVNMotorLegacy_h
#define EVNMotorLegacy_h
...
#include "evn_motor_defs_legacy.h"
#include <cfloat>
...
} pid_control_legacy_t;

class EVNMotorLegacy
{
private:
...
public:
	friend class EVNDrivebaseLegacy;
	friend class EVNOmniDrivebaseLegacy;

	EVNMotorLegacy(uint8_t port, uint8_t motortype = EV3_LARGE, uint8_t motor_dir = DIRECT, uint8_t enc_dir = DIRECT);
	void begin();
...
protected:
	...
	pid_control_legacy_t _pid_control;
	encoder_state_legacy_t _encoder;

	static bool ports_started[4];
	static encoder_state_legacy_t* encoderArgs[4]; // static list of pointers to each instances' structs
	static pid_control_legacy_t* pidArgs[4];

	static void rpm_update(encoder_state_legacy_t* arg)
	{
	...
	static void enc_update(encoder_state_legacy_t* arg)
	{
		uint8_t new_p0 = digitalRead(arg->enca);
	...
	static void writePWM_static(pid_control_legacy_t* pidArg, double speed)
	{
		//clean input
		double speedc = constrain(speed, -1, 1);

		// if button is set to enable motors / button is unlinked from motors
		if (EVNAlpha::sharedButton().read() || !EVNAlpha::sharedButton().sharedState()->link_motors)
		{
			if (speedc == 0)
			{
	...
	static void stopAction_static(pid_control_legacy_t* pidArg, encoder_state_legacy_t* encoderArg)
	{
	...
	static double getAbsPos_static(encoder_state_legacy_t* arg)
	{
		// resolution of encoder readout is in CPR (4 * PPR)
		return (double)arg->position * 360 / (4 * arg->ppr);
	}

	static double getRPM_static(encoder_state_legacy_t* arg)
	{
		uint64_t now_us = micros();
	...
	static void pid_update(pid_control_legacy_t* pidArg, encoder_state_legacy_t* encoderArg)
	{
	...
	static void attach_interrupts(encoder_state_legacy_t* encoderArg, pid_control_legacy_t* pidArg)
	{
...
class EVNDrivebaseLegacy
{
public:
	EVNDrivebaseLegacy(uint32_t wheel_dia, uint32_t wheel_dist, EVNMotorLegacy* _motor_left, EVNMotorLegacy* _motor_right);
	void begin()
	{
...
private:
	EVNMotorLegacy* _left;
	EVNMotorLegacy* _right;
	double _max_rpm;
};

class EVNOmniDrivebaseLegacy
{
public:
	EVNOmniDrivebaseLegacy(uint32_t wheel_dia, uint32_t wheel_dist, EVNMotorLegacy* fl, EVNMotorLegacy* fr, EVNMotorLegacy* bl, EVNMotorLegacy* br)
	{
		_fl = fl;
		_fr = fr;
		_bl = bl;
