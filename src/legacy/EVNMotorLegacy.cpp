#include "EVNMotorLegacy.h"

encoder_state_legacy_t* EVNMotorLegacy::encoderArgs[];
pid_control_legacy_t* EVNMotorLegacy::pidArgs[];
bool EVNMotorLegacy::ports_started[] = { false, false, false, false };

EVNMotorLegacy::EVNMotorLegacy(uint8_t port, uint8_t motortype, uint8_t motor_dir, uint8_t enc_dir)
{
	// clean inputs
...
void EVNMotorLegacy::setSpeedPID(double p, double i, double d)
{
	_pid_control.speed_pid->setKp(p);
	_pid_control.speed_pid->setKi(i);
	_pid_control.speed_pid->setKd(d);
}
void EVNMotorLegacy::setPosPID(double p, double i, double d)
{
...
void EVNMotorLegacy::setAccel(double accel_rpm_per_s) { _pid_control.accel = fabs(accel_rpm_per_s); }
void EVNMotorLegacy::setDecel(double decel_rpm_per_s) { _pid_control.decel = fabs(decel_rpm_per_s); }
void EVNMotorLegacy::setMaxRPM(double max_rpm) { _pid_control.max_rpm = fabs(max_rpm); }
void EVNMotorLegacy::setPPR(uint32_t ppr) { _encoder.ppr = ppr; }
double EVNMotorLegacy::getPos() { return getAbsPos_static(&_encoder) - _position_offset; }
void EVNMotorLegacy::resetPos() { _position_offset = getAbsPos_static(&_encoder); }
double EVNMotorLegacy::getRPM() { return getRPM_static(&_encoder); }

void EVNMotorLegacy::writePWM(double speed)
{
	_pid_control.run_speed = false;
	_pid_control.run_time = false;
	_pid_control.run_deg = false;
...
void EVNMotorLegacy::runSpeed(double rpm)
{
...
void EVNMotorLegacy::runPosition(double rpm, double position, uint8_t stop_action, bool wait)
{
	double degrees = position - this->getPos();
	this->runDegrees(rpm, degrees, stop_action, wait);
}

void EVNMotorLegacy::runDegrees(double rpm, double degrees, uint8_t stop_action, bool wait)
{
	double rpmc = constrain(rpm, -_pid_control.max_rpm, _pid_control.max_rpm);
...
void EVNMotorLegacy::runTime(double rpm, uint32_t time_ms, uint8_t stop_action, bool wait)
{
	double rpmc = constrain(rpm, -_pid_control.max_rpm, _pid_control.max_rpm);
	uint8_t stop_actionc = min(3, stop_action);
